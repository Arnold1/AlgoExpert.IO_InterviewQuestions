...
namespace _4.BranchSums
{
	...
	class Program
    {
		...
		static void CalculateBranchSums(BinaryTree node, int runningSum, List<int> BranchSums)
        {
            if (node == null)
            {
				return;
			...
			int newRunnungSum = runningSum + node.value;
			...
			CalculateBranchSums(node.left, newRunnungSum, BranchSums);
			CalculateBranchSums(node.right, newRunnungSum, BranchSums);
